...
def main():
    ...
    for iter_no in range(0, 100):
        ...
        tmp1 = (1 / (len(data_x))) * sum([(t0 + t1 * data_x[i] - data_y[i]) for i in range(len(data_x))])
        tmp2 = (1 / (len(data_x))) * sum([data_x[i] * (t0 + t1 * data_x[i] - data_y[i]) for i in range(len(data_x))])
